...
def explode( cat ) :
    ...
    desc:
        Pushes elemenst of the list onto the stack in left-to-right order
...
def list_to_hash( cat ) :
    ...
    desc:
        ...
        base: a list of pairs where each pair consists of two members:
...
def as_list( cat ) :
    ...
    desc:
        casts a variant to a (Python) list
...
def bin_op( cat ) :
    '''
    bin_op : (list:lhs list:rhs func:operation -> list:res)
...
def hash_contains( cat ) :
    ...
    tags:
        hash,dictionary,hash_table,member,contains
    '''
    key  = cat.stack.pop()
    dict = cat.stack.peek()
...
def map( cat ):
    '''
    map : (list:base func:transform -> list:new)
...
def filter( cat ) :
    '''
    filter : ( list:base func:test -> list:new )
...
def _list( cat ) :
    '''
    list : (func:generator -> list:result)
...
def dotProduct( cat ) :
    '''
    dot_prod : (list:l1 list:l2 -> float:sum)
    ...
    desc:
        ...
        sum: the dot product
    tags:
        list,vector,product,dot_product
    '''
